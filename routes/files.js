...
router.post("/upload", requireAuth({ mode: "json" }), (req, res) => {
  upload.array("files", 10)(req, res, async (uploadErr) => {
    ...
    if (!csrfValidation.valid) {
      const cleanup = cleanupUploadFiles(req.files || [], "csrf-validation-failed");
    ...
    try {
      ...
      if (!ALLOWED_FILETYPES.includes(filetype)) {
        const cleanup = cleanupUploadFiles(files, "invalid-filetype");
        if (cleanup.failed.length) {
      ...
      if (invalidFiles.length > 0) {
        if (files.length === 1) {
          ...
          const cleanup = cleanupUploadFiles(files, "mime-type-mismatch-single");
          if (cleanup.failed.length) {
          ...
          const cleanup = cleanupUploadFiles(invalidFiles, "mime-type-mismatch-batch");
          if (cleanup.failed.length) {
            logger.warn(`[Upload] MIME validation cleanup failed for ${cleanup.failed.length} file(s)`, cleanup.failed);
          }
      ...
      if (systemSettings.userStorageQuota > 0) {
        ...
        if (currentUsage + totalIncoming > systemSettings.userStorageQuota) {
          const cleanup = cleanupUploadFiles(files, "user-quota-exceeded");
          if (cleanup.failed.length) {
      ...
      if (systemSettings.branchStorageQuota > 0 && user.branch) {
        ...
        if (branchUsage + totalIncoming > systemSettings.branchStorageQuota) {
          const cleanup = cleanupUploadFiles(files, "branch-quota-exceeded");
          if (cleanup.failed.length) {
      ...
      for (const file of files) {
        ...
        if (recentDuplicate) {
          const cleanup = cleanupUploadFiles([file], "duplicate-detected");
          if (cleanup.failed.length) {
      ...
      const cleanup = cleanupUploadFiles(files, "upload-exception");
      if (cleanup.failed.length) {
  ...
  try {
    ...
    for (const f of files) {
      ...
      try {
        ...
        logger.error('[Bulk Download] Could not stat file', { filename: f.filename, error: statErr.message });
...
router.post("/recycle-bin/purge/:fileId", requireAuth({ mode: "json" }), async (req, res) => {
  try {
    ...
    const cleanup = cleanupUploadFiles(filesToClean, "recycle-bin-purge");
